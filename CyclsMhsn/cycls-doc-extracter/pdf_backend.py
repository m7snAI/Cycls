"""PDF backend for cycls_doc_extracter, via pdf-inspector.
...
since the published 1.15.0 wheel has none of the fixes below.
...
def extract_pdf(pdf_bytes: bytes, mode: str) -> dict:
    """
    ...
    Never raises — a bad PDF, a parser crash, or an import failure is
    caught and returned as an error dict instead, so a
    `cycls.remote(...).map()` batch keeps going past one bad document.
    """
    import os
    import sys
